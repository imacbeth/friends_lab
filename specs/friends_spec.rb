...
def test_likes_to_eat_false
 ...
 def test_add_friend_5
   add_friend(@person5, "Bob")
   assert_equal(1, @person5[:friends].length())
 end

 # def test_add_friend_4
 #   add_friend(@person4, "Bob")
...
def test_lend_monies_lend
  result = lend_monies(@person4, @person2, 10)
  assert_equal(10, @person4[:monies])
  assert_equal(12, @person2[:monies])
end
  # 8. Find the set of everyone's favourite food joined together
  # (hint: concatenate the favourites/snack arrays together)
def test_
  resilt = all_favorr(€people)
  expected = ["charcutery", "", "",]
end
